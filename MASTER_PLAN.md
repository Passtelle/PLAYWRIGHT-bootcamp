# 🎯 8-Week AI-Driven QA Architect Bootcamp

**Student:** Senior QA Engineer (IBM/Fiserv veteran, 10-year gap)  
**Target Role:** Senior AI-Driven QA Engineer/Architect  
**Deadline:** May 15th 2026  
**Intensity:** 8-10 hours/day  
**Current Week:** Week 2

---

## 📊 Weekly Progress Tracker

### **Week 1: Foundation & Async Mastery**

**Status:** 🟡 Completed
**Grade:** TBD  
**Focus Areas:**

- ✅ Environment setup complete (Node v24, Playwright 1.58.1)
- ✅ Async/Await mental model (COMPLETED)
- ✅ Variables: const vs let (COMPLETED)
- ✅ Arrays & basic loops (COMPLETED)
- ✅ Git basics (add/commit/push/branch) (COMPLETED)
- ✅ TypeScript explicit types (COMPLETED)
- ⏳ Git workflow (pull/merge)

**Daily Breakdown:**

- **Day 1:** JavaScript Event Loop & Async/Await ✅
- **Day 2:** Variables, Arrays, First Loop Test ✅
- **Day 3:** Loops (for...of), search multiple products test, .first() for strict mode ✅
- **Day 4:** Loop practice from memory, Git basics (status/add/commit/push) ✅
- **Day 5:** Git branches (checkout/push), TypeScript explicit types (string/number/Locator) ✅
- **Day 6:** Git workflow, final loop practice (IN PROGRESS)
- **Day 7:** Week 1 checkpoint & grade

**Day 2 Accomplishments:**

- ✅ Mastered const vs let distinction
- ✅ Understood the Trinity: Locator/Action/Assertion
- ✅ Created first loop test (day2_loop_users.spec.ts)
- ✅ Learned to prompt Claude Code effectively
- ✅ Researched modern test management tools (Jira/Xray/Zephyr)
- ✅ Understood unique data generation (timestamps for emails)

**Day 3 Accomplishments:**

- ✅ Understood for...of loop pattern (array → loop variable → actions)
- ✅ Learned .first() to handle Playwright strict mode (multiple elements)
- ✅ Created search test with loop (day3_search_products.spec.ts)
- ✅ Debugged strict mode violation independently
- ✅ Applied CLAUDE.md standards (remembered to use .first())
- ✅ Understood that test flow isn't rigid (can mix Locator/Action/Assertion order)

**Day 4 Accomplishments:**

- ✅ Wrote complete loop from memory with 95% accuracy (forgot string[] vs string)
- ✅ Learned Style A naming convention (productList/item pattern)
- ✅ Mastered Git daily workflow (status → add → commit → push)
- ✅ Fixed syntax errors in test files
- ✅ Practiced loop exercises with phone validation scenario
- ✅ Understood singular vs plural variable naming in loops
- ✅ Cleaned up broken test files from repository

**Day 5 Accomplishments:**

- ✅ Created first Git branch (day5-typescript-practice)
- ✅ Pushed branch to GitHub successfully
- ✅ Learned git checkout -b vs git push -u origin
- ✅ Understood TypeScript explicit types (string, number, string[], Locator)
- ✅ Learned what "infer" means (TypeScript figuring out types automatically)
- ✅ Understood why explicit types are better (catch errors before running)
- ✅ Saw branch on GitHub and understood repository vs branch concept
- ✅ Practiced loops from memory (90%+ correct, just type syntax slip)
- ✅ Made disciplined decision to postpone side project (Moltbook bot) until after bootcamp

**Day 6 Accomplishments:**

- ✅ Completed 3 fill-in-blank exercises (99% accuracy)
- ✅ Generated first test with Claude Code
- ✅ Learned Locator variables (professional approach)
- ✅ Identified gaps in teaching approach (Architect vs Junior)
- ✅ Discovered Arena.ai for AI comparison

**Day 7 Accomplishments:**

- ✅ Week 1 Review Quiz (82.7% average)
- ✅ Successfully used Claude Code with Architect prompt
- ✅ Gemini generated first POM (SearchPage.ts)
- ✅ Learned state-based waits (Claude Code auto-fix)
- ✅ Identified need for POM training (Week 2 focus shift)
- ✅ Made disciplined decision to start Architect phase early

**Day 8 Accomplishments:**

- ✅ Learned Page Object Model (POM) fundamentals with Gemini
- ✅ Understood HTML-to-TypeScript correlation
- ✅ Discovered Playwright locator priority (Gold/Silver/Bronze standards)
- ✅ Created first POM file (LoginPage.ts) with Claude Code
- ✅ Learned constructor, readonly, properties vs methods
- ✅ Identified actual job task breakdown: 35% Audit, 30% Architecture, 25% Prompting, 10% Syntax
- ✅ Received 2nd LinkedIn recruiter message ($50-$100/hr Web3 QA role at Tirios)
- ✅ Defined AI team roles: You (Architect), Claude Code (Builder), Claude AI (Teacher), Gemini (Judge)

**Day 9 Accomplishments:**

- ✅ Created ProductPage POM using YAGNI principle (You Aren't Gonna Need It)
- ✅ Learned to only map locators the test actually needs, not entire pages
- ✅ Built multi-POM test (SearchPage + ProductPage working together)
- ✅ Mastered Architect prompt template (Architecture/Data/Logic structure)
- ✅ Updated CLAUDE.md with standardized prompt template for team use
- ✅ Created data-driven loop tests with multiple invalid emails
- ✅ All POMs and tests passed Gemini audit (production-ready code)
- ✅ Learned chaining POMs in realistic user workflows
- ✅ 10.5 hour intensive learning day

**Day 10-11 Accomplishments:**

- ✅ Learned error message reading (5 common Playwright errors)
- ✅ Decoded real timeout error (out of stock product)
- ✅ Created POM prompt template with RegEx patterns
- ✅ Identified POM anti-patterns (no "if exists" logic, no assertions in POMs)
- ✅ Defined POM separation of concerns rules
- ✅ Created ProductPageSmart2 with proper architecture
- ✅ Built multi-POM test with conditional logic (if in stock / if out of stock)
- ✅ Established strict prompt templates with Gemini (Section 8 in CLAUDE.md)
- ✅ Switched to practicesoftwaretesting.com (better modern practice site)
- ✅ Updated all documentation with final POM standards
- ✅ Took 2-day rest after intense 10-hour session (self-care + recovery)
- ⏳ Behind schedule - Fixtures moved to Day 12-13

**Day 12 Accomplishments:**

- ✅ Built ProductQuantityPage POM with + and - buttons for quantity control
- ✅ Learned variable naming flexibility (quantityIncreaseButton vs plusButton - both valid)
- ✅ Generated test with relaxed prompt - Claude Code figured out structure
- ✅ Discovered Claude Code learns from existing tests (copied old site URL)
- ✅ Fixed AI-generated code with simple corrective prompt
- ✅ Practiced critical code review (spotted assertions going around POM)
- ✅ Identified toast notification verification challenge
- ✅ Recognized cart state problem (need clean starting state)
- ✅ Connected to past experience: "Like re-ghosting machines for security testing!"
- ✅ Previewed Fixtures need (Day 13 topic)
- ✅ Maintained warm coaching relationship after calling out robotic responses

**Day 13 Accomplishments:**

- ✅ Built complete cart test with Fixtures (day13_cart_fixture_test.spec.ts)
- ✅ Mastered beforeEach/afterEach hooks for test independence
- ✅ Used state-based waiting pattern (try-catch with waitFor instead of isVisible)
- ✅ Learned conditional cart clearing (check visibility before clearing)
- ✅ Updated CLAUDE.md Section 5: Avoid isVisible() - use waitFor with timeout
- ✅ Discovered cart persists between test runs (site clears cart automatically)
- ✅ Test passes reliably across multiple runs (3/3 passed)
- ✅ Portfolio-ready test demonstrating proper fixture usage
- ✅ Researched real-world practice sites beyond retail
- ✅ Selected automationintesting.online as Week 3 practice site

**Day 14 Accomplishments (Friday March 13):**

- ✅ Switched to automationintesting.online (B&B booking platform - realistic SPA)
- ✅ Built ContactPage.ts POM with explicit prompts (production-quality)
- ✅ Created contact form E2E test (day14_contact_form.spec.ts) - 3/3 passed
- ✅ Built AdminLoginPage.ts POM independently (95% accuracy on first attempt)
- ✅ Created admin login E2E test (day14_admin_login.spec.ts) - 3/3 passed
- ✅ Locked in explicit prompt template (Architecture/Data/Logic/Constraints)
- ✅ QA Training: Test case writing in Jira format (TC-02, TC-03)
- ✅ Identified UX bugs in contact form (form disappears after submit)
- ✅ Challenged both coach AND Gemini on incorrect assumptions
- ✅ Applied YAGNI principle (trusted Claude Code to handle import paths)
- ✅ Explored booking flow (date picker research for Day 15)

**Day 14-15 Weekend Accomplishments (March 14-15):**

- 🏆 **EARNED 6 ANTHROPIC AI CERTIFICATIONS IN 2 DAYS:**
  1. ✅ Claude 101 (9/10 score)
  2. ✅ Claude Code in Action (7/8 score)
  3. ✅ AI Fluency: Framework & Foundations
  4. ✅ Introduction to Model Context Protocol
  5. ✅ Introduction to Agent Skills
  6. ✅ Model Context Protocol: Advanced Topics
- ✅ Added all 6 certifications to LinkedIn profile
- ✅ Mastered hooks concept (scored 7/8 despite "complex" questions)
- ✅ Learned MCP architecture and workflow patterns
- ✅ Understood agent skills and orchestration
- ✅ Decided on ISTQB certification timeline (Week 8, perfect for interview prep)
- ✅ Realized Claude Code should be treated as Senior Developer, not code generator

---

### **Week 2: Playwright Architecture**

**Status:** ✅ Completed
**Grade:** A+ (Outstanding progress with AI orchestration)
**Focus Areas:**

- ✅ Page Object Model (POM) from scratch
- ✅ Fixtures & Test Hooks
- ✅ AI Test Generation with explicit prompts
- ✅ Multi-site practice (practicesoftwaretesting.com → automationintesting.online)
- ✅ Database setup for backend verification (MCP preparation)

**Week 2 Summary:**
Completed transition from manual coding to AI-assisted architecture. Established explicit prompt template for consistent POM generation. Switched practice sites strategically to automationintesting.online for realistic booking flows. Set up complete MCP infrastructure (SQLite database + MCP server) for backend verification workflows. Orchestrated 4-person AI team (Gemini + Claude Code + Coach + Ingrid) demonstrating 2026 QA Architect model.

**Daily Breakdown:**

- **Day 8-10:** Built POM structure for practicesoftwaretesting.com (ProductPage, CartPage)
- **Day 11-12:** Setup/teardown patterns, fixtures deep dive
- **Day 13:** AI-assisted test generation, established prompt standards
- **Day 14:**
  - ✅ Switched to automationintesting.online (modern React SPA)
  - ✅ Locked in explicit POM prompt template (4 sections: Architecture, Data, Logic, Constraints)
  - ✅ Created ContactPage.ts + AdminLoginPage.ts POMs
  - ✅ Wrote test cases in Jira format (TC-001, TC-002, TC-003)
  - ✅ Discovered UX bug (form disappears after submission)
  - ✅ Test suite design thinking (1 feature → 6+ test cases)
  - ✅ Verified expert assumptions (hash routing vs HTML5 History API)
- **Day 14-15 (Weekend):**
  - ✅ Earned 6 Anthropic AI certifications
  - ✅ Scheduled ISTQB Foundation exam (Week 8)
  - ✅ Updated LinkedIn profile with certifications
- **Day 15:**
  - ✅ Created BookingPage.ts POM with date picker strategy
  - ✅ Wrote day15_booking-Step1.spec.ts (search dates → verify "Our Rooms" appears)
  - ✅ Caught weak assertion bug (testing Playwright vs testing application)
  - ✅ Set up SQLite database (`hotel_backend.db`) with bookings table
  - ✅ Configured MCP SQLite server (`.mcp.json`)
  - ✅ Debugged Windows-specific issues (npx.cmd, package 404)
  - ✅ Successfully connected Claude Code to database via MCP
  - ✅ Verified autonomous database queries work
  - ✅ Team collaboration: Gemini (architect) + Claude Code (dev) + Coach (teacher) + Ingrid (PM)

**Key Files Created Week 2:**

- `pages/ContactPage.ts`
- `pages/AdminLoginPage.ts`
- `pages/BookingPage.ts`
- `tests/bootcamp/day14_contact_form.spec.ts`
- `tests/bootcamp/day14_admin_login.spec.ts`
- `tests/bootcamp/day15_booking-Step1.spec.ts`
- `hotel_backend.db` (SQLite database)
- `.mcp.json` (MCP server configuration)

**Skills Mastered:**

- ✅ Explicit prompt engineering for AI code generation
- ✅ POM architecture with YAGNI principle
- ✅ Test case design (boundary testing, validation, edge cases)
- ✅ Multi-AI orchestration (4-person team model)
- ✅ Database setup for E2E verification
- ✅ MCP server configuration
- ✅ Platform-specific debugging (Windows)
- ✅ Conversational Driven Development (CDD)
- ✅ Senior-level assumption verification

**Certifications Earned:**

1. Claude 101 (9/10)
2. Claude Code in Action (7/8)
3. AI Fluency: Framework & Foundations
4. Introduction to Model Context Protocol
5. Introduction to Agent Skills
6. Model Context Protocol: Advanced Topics

**Week 2 Grade Justification:**
Demonstrated senior-level thinking by: (1) Challenging coach's recommendation on MCP with research-backed reasoning, (2) Identifying weak test assertions that test tools vs application, (3) Verifying expert assumptions through actual testing, (4) Orchestrating multi-AI team effectively, (5) Debugging platform-specific issues independently, (6) Thinking in test suites not individual tests (1 feature → 6+ cases).

**AI Team Structure (Updated Day 15):**

1. **Human Architect (Ingrid):** PM/Tech Lead - defines WHAT to build, sets quality standards, reviews architecture, approves final implementation
2. **Claude Code (Senior Developer):** Explores sites independently, designs solutions, implements code following CLAUDE.md, self-reviews, explains design decisions
3. **Claude AI (Coach/Teacher - Me):** Explains concepts, provides lessons, builds understanding, challenges assumptions, keeps architect accountable
4. **Gemini (Master Judge/Jury):** Audits against CLAUDE.md standards, reviews complex architectural decisions, provides strategic guidance

**Portfolio Projects Completed:**

1. ✅ ContactPage.ts + contact form E2E test
2. ✅ AdminLoginPage.ts + admin login E2E test
3. ✅ Cart management test with fixtures
4. ✅ Product search with loops
5. ✅ Multi-POM workflows

**Certifications Earned:**

1. ✅ Claude 101 (Anthropic, March 14, 2026)
2. ✅ Claude Code in Action (Anthropic, March 14, 2026)
3. ✅ AI Fluency: Framework & Foundations (Anthropic, March 14, 2026)
4. ✅ Introduction to Model Context Protocol (Anthropic, March 14, 2026)
5. ✅ Introduction to Agent Skills (Anthropic, March 14, 2026)
6. ✅ Model Context Protocol: Advanced Topics (Anthropic, March 15, 2026)

---

### **Week 3: AI-Driven Workflows & Code Auditing**

**Status:** 🟡 In Progress - **MAJOR BREAKTHROUGH Day 17**
**Focus Areas:**

- ✅ Playwright Codegen (recording workflow)
- ✅ Claude Code `/plan` mode mastery
- ✅ Autonomous page scouting (web fetch)
- ✅ AI code auditing methodology
- ✅ QA_AUDIT_LESSONS.md training system
- ⏳ Test writing & negative scenario practice
- ⏳ LoginPage audit session

**Week 3 Pivot:**
Shifted from "E2E booking suite" to **AI Auditing focus** after discovering this is the core 2026 QA Architect skill. Switched practice site from automationintesting.online (poor architecture) to ParaBank (proper banking structure, matches Fiserv background).

**Daily Breakdown:**

- **Day 16:**
  - ✅ Pivoted to Digital Wallet MCP project (financial testing)
  - ✅ Created `wallet_backend.db` with Users/Transactions tables
  - ✅ Built `init_wallet_db.js` (idempotent seeding script)
  - ✅ Generated `src/wallet.ts` business logic (deposit/withdraw/transfer)
  - ✅ Strategic test user design (Alice/Bob/Carol pattern)
  - ✅ Coaching philosophy shift: Understanding over speed
- **Day 17:** **BREAKTHROUGH DAY** 🔥
  - ✅ Learned Playwright Codegen (scout → record → generate)
  - ✅ Mastered Claude Code `/plan` mode (research before coding)
  - ✅ Prompted AI to autonomously fetch/analyze ParaBank registration page
  - ✅ Created ParabankLoginPage.ts (semantic locators, explicit types)
  - ✅ Created ParabankRegisterPage.ts (11 explicit parameters)
  - ✅ **Spotted AI design flaw:** Password mismatch gap (couldn't test negative scenario)
  - ✅ **Challenged AI's solution:** Optional default vs required param
  - ✅ **Taught AI better approach:** Explicit over convenient (QA mindset wins)
  - ✅ **Updated CLAUDE.md Section 7:** "Method Parameters Must Be Fully Explicit" rule
  - ✅ **Created QA_AUDIT_LESSONS.md:** New audit training documentation system
  - ✅ Built Locator Priority Cheat Sheet (Gold/Silver/Bronze tiers)
  - ✅ Established AI-Driven QA workflow: Generate → Audit → Challenge → Fix → Document
- - **Day 18:** ✅ COMPLETE (Friday March 20)
  - ✅ Installed Faker.js (@faker-js/faker)
  - ✅ Created helpers/testData.ts (US locale, timestamp usernames)
  - ✅ Built 3-test registration suite (happy/negative/edge cases)
  - ✅ Used test.describe and beforeEach hooks
  - ✅ Debugged Spring MVC locator issue (customer.firstName pattern)
  - ✅ Fixed ParabankRegisterPage.ts locators
  - ✅ All tests passing (3/3 in 13.8s)
  - ✅ Learned: Happy Path, Negative Path, Edge Case terminology
  - ✅ Learned: test.describe, beforeEach, test isolation concepts
  - ✅ Identified phone field validation bug (accepts 16+ digits)
  - ✅ Learned HTML input type best practices (text vs number for SSN)

- **Day 19:** ✅ COMPLETE (Monday March 23)
  - ✅ Updated all docs (MASTER_PLAN, QA_AUDIT_LESSONS, created INTERVIEW_PRACTICE.md)
  - ✅ Installed dotenv, wired up .env with PARABANK_USERNAME/PASSWORD
  - ✅ Built ParabankLoginPage.ts POM (logout method included)
  - ✅ Created parabank_login.spec.ts — 2 tests: valid login + invalid password (2/2 passing)
  - ✅ Daily interview practice started — Session 1: WireWheel Data Factory pattern
  - ✅ Gemini audit passed for login POM

- **Day 20:** ✅ COMPLETE (Tuesday March 24)
  - ✅ Built ParabankOpenAccountPage.ts POM (openAccount, getNewAccountId methods)
  - ✅ AccountType union type: 'CHECKING' | 'SAVINGS'
  - ✅ getByLabel() failure diagnosed — Parabank table layout has no label associations
  - ✅ QA_AUDIT_LESSONS.md Lesson 10 added (getByLabel fails on legacy apps)
  - ✅ Strategic decision: retire Parabank for UI testing, keep lessons as interview gold
  - ✅ Two-app stack chosen: QA Playground (UI) + DummyJSON (API)
  - ✅ VS Code fully equipped: Playwright Test, ESLint, Prettier, Copilot, indent-rainbow, Markdown Preview Enhanced, emojisense

- **Day 21:** ✅ COMPLETE (Wednesday March 25)
  - ✅ API testing track begins — Postman manual lab (DAY21_API_POSTMAN_LESSON.md)
  - ✅ Platform pivots: ReqRes.in → retired (became paid BaaS), DummyJSON chosen
  - ✅ Stage 1 complete: GET all users, single user, search with query params
  - ✅ Stage 2 complete: JSON flat vs nested, navigating Emily's bank/address/company data
  - ✅ Stage 3 complete: Status codes — triggered 404 (user 99999) + 401 (auth/me no token)
  - ✅ Stage 4 partial: POST login → 200 + accessToken received
  - ✅ QA_AUDIT_LESSONS.md Lesson 11 added (401 vs 403 label trap)
  - ✅ REST vs JSON mental model established (REST = rules, JSON = language)
  - ✅ Pagination, query params, nested objects, JWT tokens all understood
  - ⏳ Resume Day 21: Exercise 4.2 (POST /users/add → expect 201) then Stages 5–7

**Key Files Created Week 3:**

- `wallet_backend.db`, `scripts/init_wallet_db.js`, `src/wallet.ts`
- `pages/ParabankLoginPage.ts`
- `pages/ParabankRegisterPage.ts`
- `QA_AUDIT_LESSONS.md` (audit training system)

**Skills Mastered Week 3:**

- ✅ Playwright Codegen workflow
- ✅ Claude Code `/plan` mode
- ✅ Autonomous AI scouting (web fetch)
- ✅ AI code auditing ("What scenarios does this NOT handle?")
- ✅ Challenging AI architectural decisions
- ✅ Teaching AI better patterns (explicit params principle)
- ✅ Building reusable training documentation
- ✅ Strategic test data design (happy/boundary/negative users)

**Daily Breakdown:**

- **Day XX:** ✅ MCP SQLite server setup complete, database connected

---

### **Week 4: API Testing + Jira + Test Case Design**

**Status:** 🟡 In Progress
**Focus Areas:**

- Postman deep dive (real job usage, not overview)
- Playwright API automation
- API/Auth helper setup (reusable login helper)
- Jira + Xray basics (test case management)
- Test case design practice (all 4 layers)
- Daily: 15 min TS warm-up + 30 min prompting practice + 1 interview question

**Carried Forward from Week 3:**

- ⏳ Postman Stage 5.2 (environment variable for token)
- ⏳ Postman Stages 6-7 (test scripts + collection organization)
- ⏳ Interview practice: job description analysis session

**Note: CI/CD moved to Week 5 (earlier than original Week 6 plan)**

**Daily Breakdown:**

- **Day 21:** ✅ Postman manual lab — DummyJSON, Stages 1-5.1 complete (see Week 3)
- **Day 22:** ✅ Planning day — revised curriculum, ISTQB strategy, test case writing practice
- **Day 23:** ✅ Postman deep dive — real-world workflow, 10 testing layers, contract testing, pre/post scripts, pm.test intro
- **Day 24:** ✅ CoinGecko pivot — DevTools + Postman connected, first pm.test written (green)
- **Day 25:** ✅ Gemini Core 6 lesson — Chai + pm object understood, Snippets tab, WireWheel interview answer
- **Day 26-27:** ✅ Postman Layers 1 & 2 — DummyJSON Login, Bearer Token flow, Happy Path + Negative Testing (took 2 days due to teaching adjustment)
- **Day 28:** ✅ Postman Layers 3 & 4 — Boundary Testing + Security Testing against DummyJSON auth endpoints
- **Day 29:** ✅ Jira & Xray Intensive (in progress) — Project created, User Story written, Xray installed, BDD concepts established
- **Day 30:** Playwright API Automation — translate Postman tests into TypeScript framework
- **Day 31:** Week 4 Checkpoint & Framework Cleanup — unify code, ready for GitHub

**Day 32 Accomplishments (April 22, 2026):**

- ✅ POM architecture refresher — reconnected mental model after API testing detour
- ✅ Caught CC's wrong locators on SecureBankLoginPage.ts (email vs username, Sign In vs Login)
- ✅ Discovered CLAUDE.md locator section was incomplete — Gold/Silver/Bronze tiers were missing
- ✅ Updated CLAUDE.md Section 2 with full Gold/Silver/Bronze locator priority system
- ✅ Learned getByTestId is Gold tier (beats getByLabel and getByPlaceholder)
- ✅ Verified all 3 login locators in DevTools independently (username-input, password-input, login-button)
- ✅ Understood why getByLabel beats getByPlaceholder (structural vs cosmetic)
- ✅ SecureBankLoginPage.ts approved — 3 Gold locators, correct parameter names
- ✅ Investigated Dashboard HTML — found precise data-testid values for all 3 stat cards
- ✅ Learned card wrapper vs value element distinction (accounts-count-card vs accounts-count)
- ✅ Understood textContent() returns string | null — ?? '' pattern explained
- ✅ SecureBankDashboardPage.ts built — 3 Gold locators, getter methods returning Promise<string>
- ⏳ Promise<string> vs Promise<void> — lesson deferred to Day 33 (end of day fatigue)
- ⏳ SecureBankTransactionPage.ts — next session
- ⏳ E2E test wiring — next session

- ✅ Promise<string> vs Promise<void> — fully understood with French-friendly explanation
- ✅ SecureBankTransactionPage.ts built and approved — 5 Gold locators
- ✅ selectOption() — three methods: by value, by label, by RegExp
- ✅ Dynamic ID problem caught — account values are timestamp-generated, hardcoding breaks tests
- ✅ Balance-in-label problem caught — 'Primary Savings - $5,000.00' breaks after deposit
- ✅ Fix applied — RegExp partial match: selectOption({ label: /Primary Savings/ })
- ✅ E2E test generated by CC — senior style with inline RegExp assertions
- ⏳ E2E test not yet run — pending TS basics refresh session
- ⏳ TypeScript basics review needed before continuing

**Day 33 Accomplishments (April 23, 2026):**

- ✅ Promise<void> vs Promise<string> fully understood — actions return void, getters return string
- ✅ ?? '' (nullish coalescing) locked — textContent() returns string | null, ?? prevents TypeScript error
- ✅ SecureBankTransactionPage.ts built and approved — 5 Gold locators
- ✅ selectOption() three methods locked: by value, by label { label: }, by RegExp { label: /pattern/ }
- ✅ Radix UI combobox vs native <select> distinction — combobox needs click() + getByRole('option')
- ✅ Dynamic ID trap caught — timestamp-generated account IDs break when data resets
- ✅ Balance-in-label trap caught — 'Primary Savings - $5,000.00' breaks after any deposit
- ✅ Fix: selectOption({ label: /Primary Savings/ }) — RegExp partial match, stable forever
- ✅ navigateToDashboard() added to SecureBankDashboardPage — architectural gap caught during E2E build
- ✅ Hard reload vs client-side navigation — page.goto() resets React state, nav link preserves it
- ✅ E2E test passing locally — 1 passed ✅
- ✅ depositAmount as string — fill() only accepts strings, even numbers go in as '500'

**Day 34 Accomplishments (April 24, 2026):**

- ✅ CI/CD full concept lesson — what GitHub Actions does, workflow file anatomy line by line
- ✅ GitHub Secrets configured — PARABANK_USERNAME, PARABANK_PASSWORD added to vault
- ✅ Workflow file fixed — timeout 3→60, scoped to SecureBank E2E, secrets wired in
- ✅ dotenv missing from package.json — classic 'works on my machine' CI problem caught and fixed
- ✅ Pipeline green ✅ — first clean run on GitHub Actions
- ✅ JUnit XML reporter added to playwright.config.ts — Playwright now generates test-results/results.xml
- ✅ Xray API Keys generated — Client ID + Client Secret saved securely
- ✅ XRAY_CLIENT_ID + XRAY_CLIENT_SECRET added to GitHub Secrets
- ✅ Xray workflow step added — authenticate + POST JUnit XML to Xray Cloud API

**Day 35 Accomplishments (April 27, 2026):**

- ✅ Weekend off — intentional rest, brain recovery
- ✅ Xray upload debugged — exit code 26 = curl can't read file (XML not being generated)
- ✅ Root cause found: JUnit reporter couldn't write to test-results/ (directory not created when tests pass)
- ✅ Fix: moved outputFile to repo root (results.xml) — always writable, find confirms it exists
- ✅ Full pipeline green with Xray upload — BAS-16 Test Execution auto-created in Jira
- ✅ BAS-15 vs BAS-17 architecture lesson — JUnit import creates Generic test, doesn't match Cucumber spec
- ✅ Identified real enterprise problem: JUnit import creates duplicate tickets without Xray key annotation
- ✅ Interview story locked: "I found that Xray's JUnit import auto-creates Generic tests — scaling to 100 tests means 100 unmapped tickets. The fix is Xray key annotation or a native Playwright/Xray reporter."
- ✅ BAS-15 Gherkin test case audited and corrected — precondition moved to Preconditions tab, BAS-14 linked
- ✅ README review — identified gaps, planned full rewrite for Day 36 morning
- ✅ GitHub Copilot added to curriculum (Week 6)
- ⏳ README rewrite — Day 36 (fresh start, full effort)

**Week 5 Status: ✅ COMPLETED**

**Day 29 Accomplishments (April 10, 2026):**

- ✅ Superpowers plugin installed (v5.0.7) — 14 skills + 3 commands (/brainstorm, /write-plan, /execute-plan) active next session
- ✅ Jira navigation learned — "Spaces" = Projects in new Jira Cloud UI
- ✅ Banking API Suite project created (Scrum template, key: BAS)
- ✅ BAS Sprint 1 created automatically
- ✅ BAS-1 User Story created: "User can log in with valid credentials and receive an access token"
- ✅ User Story format locked: "As a [user] / I want [action] / So that [benefit]" — lives in Jira Story
- ✅ BDD format distinction: Given/When/Then lives in Xray test cases, NOT in User Story
- ✅ Acceptance Criteria distinction: old way = manual text in description; modern way (Xray) = BDD test cases ARE the AC
- ✅ Xray installed (7-day trial) — Test issue type navigation in progress
- ✅ Jira roles clarified: PM writes story content, QA owns AC + test cases; at startups QA does all of it
- ⏳ Xray BDD test cases — next session (finding Test issue type in team-managed project)
- ⏳ Bug report — next session
- ⏳ JQL basics — next session

**Day 28 Accomplishments (April 10, 2026):**

- ✅ Layer 3 (Boundary): empty username, empty password, missing field, 200-char username
- ✅ Server distinguishes between empty string and missing field (same 400, design insight)
- ✅ Long input reveals no max-length validation — design gap filed as Low-Medium defect
- ✅ Variable naming consistency locked: always use `body` as the standard convention
- ✅ Layer 4 (Security): SQL injection, `admin' --` pattern, no token, fake/malformed token
- ✅ All security tests returned 401/400 — server stable, no auth bypass, no 500
- ✅ Two 401 messages distinguished: "Access Token is required" vs "Invalid/Expired Token!"
- ✅ `.to.eql()` vs `.to.include()` — exact match vs partial match, when to use each
- ✅ Full 4-layer interview answer constructed and ready to practice
- ✅ CAREER_TARGETS.md created — Ondo Finance saved as north star target, gap analysis complete
- ✅ LinkedIn headline revised: removed CI/CD, kept AI-Augmented Testing
- ✅ MetaMask/Web3 automation added as portfolio priority project

**Day 26-27 Accomplishments (April 7-8, 2026):**

- ✅ Layer 1 (Happy Path): POST /auth/login → 200 + accessToken verified
- ✅ pm.response.json() — parsing response body into readable JS object
- ✅ Stacked pm.test blocks ("Pancake Rule") — no nesting, clean structure
- ✅ 100% pass rate (2/2) on DummyJSON Login Happy Path
- ✅ Layer 2 (Negative Path): bad credentials → 400 + error message verified
- ✅ pm.expect(body.message).to.eql('Invalid credentials') — exact contract check
- ✅ API negative testing mindset: protecting the database, not verifying the UI
- ✅ 400 vs 500 distinction: server caught bad input gracefully, did not crash
- ✅ Two interview answers locked (see INTERVIEW_PRACTICE.md Sessions 7-8)
- ✅ Teaching approach adjusted: Gemini for concept explanations, Instance A for application

**Day 25 Accomplishments (April 7, 2026):**

- ✅ Gemini Core 6 lesson — Chai and pm object fully explained
- ✅ pm = PostMan master remote control (mental model locked)
- ✅ Chai = translator that makes JavaScript readable (mental model locked)
- ✅ Core 6 assertions memorized: Status, Speed, Contract, Type, Exact Match, Array
- ✅ const jsonData = pm.response.json() — always the first line before assertions
- ✅ Snippets tab discovered — senior QAs use these, not memorization
- ✅ pm.response vs pm.expect distinction understood
- ✅ Interview answer practiced: WireWheel API speed question
- ✅ Reference file created: DAY25_POSTMAN_CORE6.md

**Day 24 Accomplishments (April 2, 2026):**

- ✅ Pivoted API practice context from DummyJSON to CoinGecko (crypto background, more motivating)
- ✅ Called live Bitcoin price directly in browser — raw JSON visible
- ✅ DevTools Network tab: Fetch/XHR filter, isolating one user action (XRP search)
- ✅ Identified real API calls vs analytics trackers (search_v2 vs i.gif)
- ✅ Connected UI and API: same data, different presentation layer
- ✅ First pm.test written independently and passed (green): "Search returns 200"
- ✅ Snippets tab identified as the correct starting point (teaching gap corrected)

**Day 23 Accomplishments (March 28-30, 2026):**

- ✅ Real-world Postman workflow — verify the contract, then break it
- ✅ 10 testing layers learned (4 foundation + 6 advanced)
- ✅ Tool mapping: Postman vs Playwright vs k6 vs DB queries
- ✅ BVA terminology confirmed (Boundary Value Analysis, not "edge cases")
- ✅ Contract testing concept — field types, not just status codes
- ✅ SQL injection example — what it looks like in a request body
- ✅ Idempotency, concurrency, audit trail, error disclosure — named and understood
- ✅ Pre-request & Post-response scripts — JavaScript, pm object, Chai assertions
- ✅ pm.test() / pm.expect() / pm.environment.set() — purpose understood
- ✅ Token auto-save pattern — post-response script vs manual environment variable
- ✅ Key mindset: syntax is Google's job, thinking is yours
- ✅ Multi-agent setup stabilized: Instance A (sidebar) + Instance B (terminal)
- ✅ user_story.md created — full personal story saved to permanent memory

**Day 22 Accomplishments (March 27-28, 2026):**

- ✅ Instance A (Coach) set up with full memory system
- ✅ Revised bootcamp deadline to May 15, 2026
- ✅ Revised curriculum agreed: CI/CD to Week 5, Copilot added, visual regression slimmed, Jira added to Week 4
- ✅ AI-Driven/Agentic testing vocabulary established — woven into every week, not a separate topic
- ✅ TypeScript spiral review plan: 15 min warm-up daily, 10 rotating topics
- ✅ Prompting practice plan: 30 min per session, architect template reps
- ✅ ISTQB strategy: practice exam first, vocabulary gaps, Chapters 2/4/5 focus
- ✅ ISTQB key vocabulary: Error/Defect/Failure, Verification/Validation, Regression/Confirmation
- ✅ Test case writing — 3 progressive exercises (login, bank transfer, password reset)
- ✅ Confirmed strength: boundary value analysis (instinct-level), state verification
- ✅ Identified gap: security testing layer (SQL injection, audit logging, PII masking, role hierarchy)
- ✅ Key lesson: Steps vs Test Cases distinction — format habit, not knowledge loss

---

### **Week 5: Portfolio Build (Apr 21-27)**

**Status:** ✅ Completed
**Focus Areas:**

- E2E Banking Flow (portfolio centerpiece)
- CI/CD pipeline with GitHub Actions
- Framework cleanup + professional README

**Daily Breakdown:**

- **Day 32 (Apr 21):** E2E Banking Flow Part 1 — Login → Get Accounts → Check Balance
- **Day 33 (Apr 22):** E2E Banking Flow Part 2 — Transfer Money → Verify Balance Updated
- **Day 34 (Apr 23):** CI/CD Pipeline — GitHub Actions, run Playwright tests on push, HTML report artifact
- **Day 35 (Apr 24):** Framework cleanup + professional README

**CUT (out of scope):**

- ~~Newman CLI~~ — mention it, don't build it
- ~~Visual regression (Percy/Applitools)~~ — not needed for hybrid QA roles
- ~~BrowserStack/Sauce Labs~~ — strategy-level knowledge sufficient
- ~~Appium/Mobile~~ — strategy-level knowledge sufficient

---

### **Week 6: Interview Machine (Apr 28 - May 4)**

**Status:** ⏳ Not Started
**Focus Areas:**

- Resume rewrite (hybrid QA Engineer framing)
- Mock interviews (2/day)
- ISTQB prep

**Daily Breakdown:**

- **Day 36-37:** Resume rewrite — quantify bootcamp wins, hybrid QA Engineer focus, IBM/Fiserv framing
- **Day 38-40:** Mock interviews — 2/day, STAR method, technical QA questions
- **Day 41-42:** ISTQB Foundation — practice exam, identify gaps, targeted study (Chapters 2/4/5)

---

### **Week 7: Launch (May 5-11)**

**Status:** ⏳ Not Started
**Focus Areas:**

- ISTQB exam
- Job applications begin

**Daily Breakdown:**

- **Day 43-44:** ISTQB final prep + take exam
- **Day 45+:** Applications (5/day) — hybrid QA Engineer roles, FinTech/blockchain startups
- **Ongoing:** Fix gaps found in mock interviews, LinkedIn polish

**MetaMask/Web3:** Optional — only if a Web3 company requests an interview. Build it in 3 targeted days then.

---

### **Job Search Target (Updated April 21, 2026)**

**Role:** Senior QA Engineer — Hybrid (Manual + Automation)
**NOT:** Pure QA Architect (too few openings, requires recent leadership)
**Why hybrid is right:**

- 10x more postings
- 15 years IBM/Fiserv manual QA = major differentiator vs automation-only candidates
- The story: enterprise-grade manual foundation + modern Playwright/API/CI/CD stack

**Job boards:** Wellfound, BuiltIn, HiringCafe
**Apply date:** May 15, 2026
**Financial runway:** June 27, 2026

**E-Trade recruiter (warm contact):** Follow up May 15 with:

> "Completed bootcamp, ISTQB certified, CI/CD pipeline live on GitHub."

---

## 🎓 Skills Checklist

### **Core Automation** (Week 1-3)

- [ ] Async/await mastery
- [ ] TypeScript type safety
- [ ] Arrays & loops
- [ ] Page Object Model
- [ ] Git workflows
- [ ] AI-assisted test generation

### **API & Data** (Week 4-5)

- [ ] Postman/Newman
- [ ] API contract testing
- [ ] Mock servers
- [ ] Visual regression
- [ ] AI model testing

### **DevOps & Strategy** (Week 6-7)

- [ ] GitHub Actions CI/CD
- [ ] Test reporting
- [ ] Mobile testing strategy
- [ ] Accessibility audits

### **Interview Ready** (Week 8)

- [ ] Technical interview skills
- [ ] System design for QA
- [ ] Behavioral storytelling
- [ ] Portfolio showcase

---

## 📝 Notes & Adjustments

**Week 1 Adjustments:**

- Switching from saucedemo to ecommerce-playground.lambdatest.io (student preference)
- Focus on understanding "why" before "how"
- Created living document system for better note-taking

**Daily Audit Practice (Starting Week 2):**

- **Duration:** 1 hour daily
- **Structure:** Review AI-generated code → Find bugs → Fix and explain
- **Complexity:** Real production-style code, not basic examples
- **Skills built:** Architectural thinking, security awareness, performance analysis, strict mode handling
- **Goal:** By Week 8, spot 8-10 bugs in complex AI-generated tests within 30 minutes

**Student Strengths:**

- 15 years QA experience (IBM/Fiserv)
- Strong conceptual understanding
- High motivation and work ethic
- Can run multi-browser test suites
- Strong pattern recognition from day trading background
- Can read and understand existing test code
- Excellent at asking clarifying questions
- Self-aware about learning style and pace needs

**AI Team Structure (Defined Day 8):**

1. **Human Architect (Ingrid):** Director, final decision maker, orchestrates AI agents
2. **Claude Code (CLI):** Builder, generates TypeScript/Playwright code, creates files
3. **Claude AI (Web):** Teacher, explains concepts, provides lessons, builds understanding
4. **Gemini (Primary Judge):** Auditor, enforces CLAUDE.md standards, strategic guidance

**🔥 CRITICAL: First Day on Any Job (Gemini's Tip):**
**Before writing ANY code, tell Claude Code: "Read all the existing tests in this repository to learn our standard locator strategies before you write anything."**
This ensures Claude Code matches the company's existing patterns instead of creating inconsistent code.

**Job Task Breakdown (Real World):**

- Verification & Auditing: 35%
- System Architecture: 30%
- Prompt Engineering: 25%
- Writing Syntax from Memory: 10%

**Learning Style & Preferences (CRITICAL - Updated Day 6):**

- **Daily commitment:** 8-10 hours/day (often 2pm-11pm, peak productivity at night)
- **Preferred learning method:** See complete working examples → Fill in blanks → Modify code → Repeat until muscle memory
- **NOT effective:** Writing from memory without examples, rushing through concepts, short 30-40 min sessions
- **TypeScript mastery timeline:** Realistic goal is 4-6 weeks of daily practice, not 1 week
- **Target role clarity:** QA Architect who DIRECTS AI to write code, maintains and verifies scripts (not pure coding role)
- **Resume skill goals:** Playwright, TypeScript (6mo equivalent), Git, CI/CD, API Testing, Jira, LLM/Agentic workflows

**Revised Daily Structure (8-10 hours):**

- **Morning (2-3 hrs):** New concept + hands-on practice with that concept
  End every single coding session with 1 Mock Interview Question to practice the Corporate QA Dialect.
- **Afternoon (3-4 hrs):** Real-world application (build tests) + research QA field/trends
- **Evening (3-4 hrs):** Deep practice (fill-blanks, modify, debug) + documentation + portfolio work

**Teaching Approach That Works:**

1. Show complete working test (student reads and understands each line)
2. Give same test with strategic blanks - student fills them in
3. Student modifies the test (change data, add functionality)
4. Repeat pattern 5-10 times across different scenarios
5. Daily TS review/practice from previous lessons (spiral learning)
6. Focus on READING code fluency first, WRITING fluency develops over weeks 4-6

**Coach Commitments:**

- Match student's 8-10 hour daily commitment (no premature break suggestions)
- Weekly research of QA field (job postings, blogs, trends) and share insights
- Provide corporate-reality focused training (what they'll actually do at work)
- Deep learning over fast learning (master concepts before moving on)
- More conversations to check understanding and adjust approach
- Update daily Master_Plan.md, lessons.md, Interview_tips.md

**Growth Areas:**

- Git fundamentals (minimal experience)
- Loop syntax variations
- TypeScript advanced features
- Interview confidence

---

## 🎯 Success Metrics

**Week-by-Week Goals:**

- - - **Week 1:** Can READ and UNDERSTAND Playwright tests, modify existing tests, fill in code blanks correctly, identify basic syntax errors in AI-generated code
- **Week 2:** Can build a POM structure independently
- **Week 3:** Can prompt AI to generate 80% of test code
- **Week 4:** Can test APIs and update LinkedIn
- **Week 5:** Can explain Judge LLM strategy in interviews
- **Week 6:** Can build a CI/CD pipeline
- **Week 7:** Can discuss mobile/a11y testing strategy
- **Week 8:** Can pass a technical interview

**Final Goal:** Land Senior AI-Driven QA Engineer role by April 30th 2026

---

**Last Updated:** Week 4, Day 28 (April 9, 2026)
**Next Review:** End of Week 4
